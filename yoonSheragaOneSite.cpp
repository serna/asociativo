...
int main(int argc, char* argv[]){
	...
	double oldx,oldy,oldz,oldxs,oldys,oldzs,oldE,newE,oldFreeE,newFreeE,movsAcc,prevMovsAcc=0,tot_E=0.0,nMeasures=0.0,perc=0.0;
	...
	for(int i=0;i<NMOVE;i++){
		...
		//printf("\nNEW %E\tOLD%E",newFreeE,oldFreeE);
		if((newE-oldE)+(newFreeE-oldFreeE)<0.0){
			...
			if(ran2(pseed)<exp(-(newFreeE-oldFreeE + newE-oldE)/T)){
				movsAcc++;
